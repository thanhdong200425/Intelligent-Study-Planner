...
export const WeeklySummary: React.FC<WeeklySummaryProps> = ({
  ...
  const findMostProductiveTimeSlot = (timeBlocks: TimeBlock[]) => {
    ...
    Object.entries(timeSlots).forEach(([timeSlot, data]) => {
      if (data.count >= 2) {
        // At least 2 sessions for statistical significance
  ...
  const getTimeEfficiency = () => {
    ...
    return Math.round(
      (summary.totalActualTime / summary.totalPredictedTime) * 100
    );
  };
  ...
  const getInsight = (): string => {
    ...
    const avgHabitStreak =
      habits.length > 0 ? habits.reduce((a, b) => a + b, 0) / habits.length : 0;
    ...
    if (avgHabitStreak > 5) {
      return `Great job maintaining your habits! Average streak: ${Math.round(
        avgHabitStreak
      )} days.`;
    }
              ...
              .map(([courseId, minutes]) => {
                ...
                const percentage =
                  summary.totalActualTime > 0
                    ? Math.round((minutes / summary.totalActualTime) * 100)
                    : 0;
