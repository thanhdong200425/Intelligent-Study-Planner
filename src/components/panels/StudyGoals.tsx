...
function GoalRow({
    ...
    return (
        <div className="border border-gray-200 rounded-xl p-4">
            ...
            <Progress aria-label={`${title} progress`} value={value} className="mt-3" />
        </div>
    );
}
    ...
    return (
        <Card className="rounded-2xl">
            ...
            <CardBody className="px-6 pb-6 pt-4 space-y-4">
                <GoalRow icon={<BookOpenCheck className="size-5" />} title="Weekly Study Hours" subtitle="32 / 40 hours" value={80} />
                <GoalRow icon={<GraduationCap className="size-5" />} title="Course Completion" subtitle="6 / 8 courses" value={75} />
                <GoalRow icon={<CheckSquare className="size-5" />} title="Assignment Success" subtitle="15 / 20 completed" value={75} />
            </CardBody>
        </Card>
    );
}
