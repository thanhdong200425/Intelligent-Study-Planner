...
const getIconForType = (type: string) => {
  switch (type) {
    case 'Pset':
      return <Pencil className='size-3 mr-1.5' />;
    case 'Coding':
      return <Monitor className='size-3 mr-1.5' />;
    case 'Reading':
      return <Book className='size-3 mr-1.5' />;
    case 'Writing':
      return <FileText className='size-3 mr-1.5' />;
    default:
      return <FileText className='size-3 mr-1.5' />;
  }
};
...
const getTypeStyle = (type: string) => {
  switch (type) {
    ...
    case 'Pset':
      return 'bg-white border-gray-200 text-gray-600 shadow-sm';
    case 'Reading':
      return 'bg-gray-100 border-gray-100 text-gray-600';
    case 'Writing':
...
export const TaskCard: React.FC<{ task: Task }> = ({ task }) => {
  return (
    <div
      className={`group p-4 border border-gray-100 rounded-xl mb-3 flex items-start gap-4 transition-all hover:border-gray-300 hover:shadow-md bg-white ${task.completed ? 'opacity-50' : ''}`}
    >
      {/* Checkbox */}
      <div
        className={`mt-1 size-5 shrink-0 rounded-[5px] border flex items-center justify-center cursor-pointer transition-colors duration-200 ${
          task.completed
      ...
      <div className='flex-1 min-w-0'>
        <h3
          className={`text-[15px] font-medium text-gray-900 mb-2.5 ${task.completed ? 'line-through text-gray-500' : ''}`}
        >
          {task.title}
        </h3>
        ...
        <div className='flex flex-wrap items-center gap-2'>
          {/* Type Tag */}
          <span
            className={`inline-flex items-center px-2 py-1 rounded-md text-[11px] font-semibold border ${getTypeStyle(task.type)}`}
          >
          ...
          {/* Priority Tag */}
          <span
            className={`inline-flex items-center px-2 py-1 rounded-md text-[11px] font-semibold ${getPriorityStyle(task.priority)}`}
          >
          ...
          {/* Subject Tag */}
          <span
            className={`inline-flex items-center px-2 py-1 rounded-md text-[11px] font-medium border ${getSubjectStyle(task.subject)}`}
          >
...
export const DeadlineCard: React.FC<{ deadline: Deadline }> = ({
  deadline,
}) => {
  const priorityColor =
  ...
  return (
    <div className='p-4 bg-gray-50 rounded-xl mb-3 border border-gray-100 hover:bg-white hover:shadow-sm transition-all group'>
      <div className='flex justify-between items-start mb-2'>
        <h4 className='font-medium text-sm text-gray-800 leading-tight group-hover:text-blue-600 transition-colors'>
          {deadline.title}
        </h4>
        <span
          className={`text-[10px] font-bold px-1.5 py-0.5 rounded border ${priorityColor}`}
        >
          {deadline.priority}
        </span>
      </div>
      <div
        className={`text-xs font-medium mb-2 ${getSubjectStyle(deadline.subject)
          .split(' ')
...
export const StatCard: React.FC<{
  ...
  return (
    <div className='bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex flex-col justify-between h-32 hover:shadow-md transition-shadow'>
      <div
        className={`size-10 rounded-lg ${bgColor} flex items-center justify-center mb-2`}
      >
